<?php $__env->startSection('title', 'Monitoring Pemesanan'); ?>
...
<div class="flex flex-col gap-6">
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-center">
        <form method="GET" action="<?php echo e(route('admin.bookings')); ?>" class="flex flex-col md:flex-row gap-3 w-full">
            <div class="flex-1">
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama penumpang, asal, atau tujuan..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-secondary text-sm"/>
            </div>
            <div class="w-full md:w-48">
                <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-secondary text-sm">
                    ...
                    <option value="pending_payment" <?php echo e(request('status') === 'pending_payment' ? 'selected' : ''); ?>>Pending Payment</option>
                    <option value="pending_verification" <?php echo e(request('status') === 'pending_verification' ? 'selected' : ''); ?>>Menunggu Konfirmasi</option>
                    <option value="booked" <?php echo e(request('status') === 'booked' ? 'selected' : ''); ?>>Booked / Paid</option>
            ...
            <?php if(request()->has('search') || request()->has('status')): ?>
                <a href="<?php echo e(route('admin.bookings')); ?>" class="border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-50 flex items-center justify-center">
                    Reset
                </a>
            <?php endif; ?>
        </form>
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    <?php $__empty_1 = true; $__currentLoopData = $bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 font-semibold uppercase text-xs">#TCK<?php echo e($booking->id); ?></td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900"><?php echo e($booking->user?->name); ?></div>
                                <div class="text-[10px] text-gray-500 uppercase"><?php echo e($booking->booking_code); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900"><?php echo e($booking->schedule?->origin); ?> → <?php echo e($booking->schedule?->destination); ?></div>
                                <div class="text-[10px] text-gray-500"><?php echo e(\Carbon\Carbon::parse($booking->schedule?->departure_time)->format('d M Y H:mm')); ?></div>
                            </td>
                             <td class="px-6 py-4 font-bold text-primary">
                                Rp <?php echo e(number_format($booking->aggregated_total ?? 0, 0, ',', '.')); ?>

                                <?php if($booking->group_count > 1): ?>
                                    <div class="text-[9px] text-gray-400">(<?php echo e($booking->group_count); ?> Kursi)</div>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4">
                                <?php if($booking->payment_proof): ?>
                                    <div class="relative group cursor-pointer w-10 h-10 rounded-lg overflow-hidden border border-gray-200 shadow-sm" 
                                         onclick="openProofModal('<?php echo e(asset('storage/' . $booking->payment_proof)); ?>')">
                                        <img src="<?php echo e(asset('storage/' . $booking->payment_proof)); ?>" class="w-full h-full object-cover">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <?php if($booking->status === 'pending_payment' || $booking->status === 'pending_verification'): ?>
                                    ...
                                    <button type="button" class="bg-gray-100 hover:bg-gray-200 text-gray-700 p-2 rounded-lg transition-all shadow-sm" title="Detail"
                                            onclick="openDetailModal(<?php echo e(json_encode($booking->load(['user', 'schedule.vehicle']))); ?>, '<?php echo e($booking->aggregated_seats); ?>', <?php echo e($booking->aggregated_total); ?>)">
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Program1\Shuttle(local)\shuttle-backend\resources\views/admin/bookings/index.blade.php ENDPATH**/ ?>