<?php $__env->startSection('title', 'Monitoring Perjalanan'); ?>
...
<div class="flex flex-col gap-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-6">
        <div class="p-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-3">
            ...
            <form method="GET" action="<?php echo e(route('admin.trips')); ?>" class="w-full md:w-48">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    <?php $__empty_1 = true; $__currentLoopData = $trips; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trip): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        ?>
                        <tr class="hover:bg-gray-50 cursor-pointer" onclick="focusTripOnMap(<?php echo e($trip->id); ?>)">
                            <td class="px-6 py-4 font-semibold">#TRP<?php echo e($trip->id); ?></td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900"><?php echo e($trip->schedule?->driver?->name); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e($trip->schedule?->vehicle?->name); ?> (<?php echo e($trip->schedule?->vehicle?->license_plate); ?>)</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900"><?php echo e($trip->schedule?->origin); ?> → <?php echo e($trip->schedule?->destination); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-900"><?php echo e(\Carbon\Carbon::parse($trip->schedule?->departure_time)->format('H:mm')); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e(\Carbon\Carbon::parse($trip->schedule?->departure_time)->format('d M Y')); ?></div>
                            </td>
                            <td class="px-6 py-4 font-mono text-xs">
                                <?php if($latestLoc): ?>
                                    <span class="text-secondary"><?php echo e($latestLoc->latitude); ?>, <?php echo e($latestLoc->longitude); ?></span>
                                    <div class="text-[10px] text-gray-400">Update: <?php echo e($latestLoc->created_at->format('H:mm:s')); ?></div>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <?php elseif(in_array($trip->status, ['on-going', 'boarding', 'delayed', 'arrived'])): ?>
                                    <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800 animate-pulse"><?php echo e(ucfirst($trip->status)); ?></span>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        mapboxgl.accessToken = <?php echo json_encode(config('services.mapbox.access_token')); ?>;
        ...
        const activeTrips = [
            <?php $__currentLoopData = $trips->whereIn('status', ['boarding', 'on-going', 'delayed', 'arrived']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                {
                    id: <?php echo e($t->id); ?>,
                    origin: '<?php echo e(addslashes($t->schedule?->origin)); ?>',
                    destination: '<?php echo e(addslashes($t->schedule?->destination)); ?>',
                    driver: '<?php echo e(addslashes($t->schedule?->driver?->name)); ?>',
                    vehicle: '<?php echo e(addslashes($t->schedule?->vehicle?->license_plate)); ?>',
                    status: '<?php echo e($t->status); ?>',
                    pickup_name: '<?php echo e(addslashes($t->schedule?->pickup_name)); ?>',
                    pickup_lat: <?php echo e($t->schedule?->pickup_lat ?? -6.2088); ?>,
                    pickup_lng: <?php echo e($t->schedule?->pickup_lng ?? 106.8456); ?>,
                    drop_off_name: '<?php echo e(addslashes($t->schedule?->drop_off_name)); ?>',
                    drop_off_lat: <?php echo e($t->schedule?->drop_off_lat ?? -6.9175); ?>,
                    drop_off_lng: <?php echo e($t->schedule?->drop_off_lng ?? 107.6191); ?>,
                    locations: [
                        <?php $__currentLoopData = $t->locations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $loc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            [<?php echo e($loc->longitude); ?>, <?php echo e($loc->latitude); ?>],
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    ],
                    passengers: [
                        <?php $__currentLoopData = $t->schedule->bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            {
                                name: '<?php echo e(addslashes($booking->user?->name)); ?>',
                                seat: '<?php echo e($booking->seat?->seat_number ?? $booking->seat_id); ?>',
                                phone: '<?php echo e(addslashes($booking->user?->phone)); ?>'
                            },
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    ]
                },
        ...
        setInterval(function() {
            fetch("<?php echo e(route('admin.trips.locations')); ?>")
                .then(res => res.json())
                .then(data => {
                    data.forEach(trip => {
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Program1\Shuttle(local)\shuttle-backend\resources\views/admin/trips/index.blade.php ENDPATH**/ ?>